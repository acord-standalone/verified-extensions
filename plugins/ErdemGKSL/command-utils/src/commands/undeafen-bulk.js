...
export default commands?.register({
  name: "undeafen-bulk",
  get displayName() {
    return i18n.format("UNDEAFEN_BULK_COMMAND_NAME");
  },
  get description() {
    return i18n.format("UNDEAFEN_BULK_COMMAND_DESCRIPTION")
  },
  predicate: ({ channel, guild }) => {
    return guild && PermissionStore.can(1535360n, guild);
  },
  execute: async ({ args, channel, reply }) => {
    ...
    if (!targetChannel || !targetChannel.guild_id)
      return reply(i18n.format("UNDEAFEN_BULK_CHANNEL_NOT_FOUND"));

    if (!PermissionStore.can(1535360n, targetChannel))
      return reply(i18n.format("UNDEAFEN_BULK_PERMISSION_DENIED"));
    ...
    if (memberIds.length === 0) return reply(i18n.format("UNDEAFEN_BULK_NO_MEMBERS"));

    for (const memberId of memberIds) {
      await GuildActions.setServerDeaf(targetChannel.guild_id, memberId, false);
      await new Promise(resolve => setTimeout(resolve, 250));
    }

    return reply(i18n.format("UNDEAFEN_BULK_SUCCESS", memberIds.length));
  },
  get groupName() {
  ...
  options: [
    {
      name: "channel",
      get displayName() {
        return i18n.format("UNDEAFEN_BULK_CHANNEL");
      },
      type: 7,
      get description() {
        return i18n.format("UNDEAFEN_BULK_CHANNEL_DESCRIPTION");
      },
      required: true,
      channelTypes: [2]
    }
