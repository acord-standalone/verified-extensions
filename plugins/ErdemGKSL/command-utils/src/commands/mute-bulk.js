...
export default commands?.register({
  name: "mute-bulk",
  get displayName() {
    return i18n.format("MUTE_BULK_COMMAND_NAME");
  },
  get description() {
  ...
  execute: async ({ args, channel, reply }) => {
    ...
    if (!targetChannel || !targetChannel.guild_id) 
      return reply(i18n.format("MUTE_BULK_CHANNEL_NOT_FOUND"));

    if (!PermissionStore.can(7341056n, targetChannel)) 
      return reply(i18n.format("MUTE_BULK_PERMISSION_DENIED"));
    ...
    if (memberIds.length === 0) return reply(i18n.format("MUTE_BULK_NO_MEMBERS"));

    for (const memberId of memberIds) {
      await GuildActions.setServerMute(targetChannel.guild_id, memberId, true);
      await new Promise(resolve => setTimeout(resolve, 250));
    }

    return reply(i18n.format("MUTE_BULK_SUCCESS", memberIds.length));
  },
  get groupName() {
  ...
  options: [
    {
      name: "channel",
      get displayName() {
        return i18n.format("MUTE_BULK_CHANNEL");
      },
      type: 7,
      get description() {
        return i18n.format("MUTE_BULK_CHANNEL_DESCRIPTION");
      },
      required: true,
      channelTypes: [2]
    }
