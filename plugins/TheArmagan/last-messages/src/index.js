...
const channelIcons = {
  "0": `
    <svg width="24" height="24" viewBox="0 0 24 24" class="icon_ecf39b" aria-label="Kanal" aria-hidden="false" role="img">
  ...
  `,
  "2": `
    <svg class="icon_ecf39b" aria-hidden="false" role="img" width="24" height="24" viewBox="0 0 24 24">
...
export default {
  load() {
    ...
    async function patchSectionContent(sectionContent, user) {
      ...
      data.forEach(i => {
        ...
        let container = dom.parse(`
          <div class="line">
            <span class="timestamp_a65714 info" acord--tooltip-content="${moment(i.date).format("DD.MM.YYYY HH:mm:ss")}">
              ${moment(i.date).format("MMM DD, YYYY HH:mm")}
            </span>
            <span class="channelMention wrapper_f46140 interactive" role="link" tabindex="0" acord--tooltip-content="${tooltip}">
              <span class="channelWithIcon">
                ${channelIcons[channel ? channel.type : 0] || ""}
                <span class="name__48cc7">${(channel ? channel.name : i.channelName) || i.possibleTooltip}</span>
              </span>
            </span>
          </div>
        `);
    ...
    subscriptions.push(
      dom.patch(
        '.userPopoutInner_f545a3 .scroller__83262:not(.acord--patched)',
        /** @param {HTMLDivElement} elm */(elm) => {
          ...
          const section = dom.parse(`
            <div class="section__6f61e">
              <h2 class="defaultColor__77578 eyebrow_e5a66c defaultColor__87d87 title_ef4a6d" data-text-variant="eyebrow">${i18n.format("LAST_MESSAGES")}</h2>
              <div class="lm--section-content thin_b1c063 scrollerBase_dc3aa9"></div>
            </div>
          `);
    ...
    subscriptions.push(
      dom.patch(
        '.userProfileModalInner__7c87d .userInfoSection__1daf8:not(.connectedAccounts_dc0a56)',
        /** @param {HTMLDivElement} elm */(elm) => {
          const user = utils.react.getProps(elm, i => i?.user).user;
          if (!user || !authentication.token) return;
          if (elm.querySelector(".lm--section-content")) return;
          ...
          const header = dom.parse(`
            <h1 class="defaultColor__77578 eyebrow_e5a66c defaultColor__87d87 userInfoSectionHeader__31b66 acord--patched" data-text-variant="eyebrow">${i18n.format("LAST_MESSAGES")}</h1>
          `);

          const contentContainer = dom.parse(`
            <div class="userInfoText__5f822 info lm--section-content thin_b1c063 scrollerBase_dc3aa9"></div>
          `);
